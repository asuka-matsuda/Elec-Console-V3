...
describe('PortalTabAdminSites.vue', () => {
  it('renders 2-pane master detail components and Divider', () => {
    const wrapper = mount(TabAdminSites)

    expect(wrapper.findComponent({ name: 'PortalMasterSiteList' }).exists()).toBe(true)
    expect(wrapper.findComponent({ name: 'PortalDetailSiteSettings' }).exists()).toBe(true)
