...
import OrganismsSiteListMaster from '../../../app/components/portal/OrganismsSiteListMaster.vue'
import type { Site } from '../../../app/types/admin'

describe('OrganismsSiteListMaster.vue', () => {
  ...
  it('renders list of sites and allows selection', async () => {
    const wrapper = mount(OrganismsSiteListMaster, {
      props: {
        sites: dummySites,
    ...
    const items = wrapper.findAllComponents({ name: 'PortalMoleculesSiteListItem' })
