      ...
      <AppToolInputPanel @reset="isResetModalOpen = true">
        ...
        <div>
          ...
          <div class="l-stack" style="gap: var(--gap-component);">
            <AppCableRowCard
              v-for="(cable, index) in inputCables"
              :key="cable.id"
              v-model="inputCables[index]!"
              :index="index"
              :removable="inputCables.length > 1"
      ...
      >
        ...
        <AppResultDetails>
          <AppResultDetailsRow label="ケーブル合計断面積">
            <strong>{{ result.totalArea.toFixed(1) }}</strong> mm²
          </AppResultDetailsRow>
