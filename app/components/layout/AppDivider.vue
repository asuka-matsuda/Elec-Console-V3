...
.c-divider {
  ...
  // Variants (Colors)
  &.has-accent-main { --divider-accent: var(--color-category-main); }
  &.has-accent-tool { --divider-accent: var(--color-category-tool); }
  &.has-accent-database { --divider-accent: var(--color-category-database); }
  &.has-accent-reference { --divider-accent: var(--color-category-reference); }
  &.has-accent-management { --divider-accent: var(--color-category-management); }
  ...
  &--horizontal {
    ...
    &.is-animated.c-divider--default::before {
      ...
      width: 30%;
      height: 100%;
      background: linear-gradient(90deg, transparent, theme-color(var(--divider-accent, var(--color-category-main)), 80%), transparent);
  ...
  &--vertical {
    ...
    &.is-animated.c-divider--default::before {
      ...
      width: 100%;
      height: 30%;
      background: linear-gradient(180deg, transparent, theme-color(var(--divider-accent, var(--color-category-main)), 80%), transparent);
      box-shadow: 0 0 8px var(--divider-accent, var(--color-category-main));
  ...
  @keyframes data-pulse-x {
    ...
    10% { opacity: 1; }
    50% { opacity: 1; }
    60% { transform: translateX(450%); opacity: 0; } // 100% + 300% travel + buffer
    100% { transform: translateX(450%); opacity: 0; } // 休止時間
  }
  ...
  @keyframes data-pulse-y {
    ...
    10% { opacity: 1; }
    50% { opacity: 1; }
    60% { transform: translateY(450%); opacity: 0; }
    100% { transform: translateY(450%); opacity: 0; } // 休止時間
  }
}
</style>
